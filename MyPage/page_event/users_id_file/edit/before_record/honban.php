...
if($values["status"] == 3){
	$kaiin_user_id = $oldvalues["user_id"];
	$sql = "UPDATE users SET identity_check=1 WHERE id={$kaiin_user_id} LIMIT 1";
	$rs = CustomQuery($sql);
}else{
	$kaiin_user_id = $oldvalues["user_id"];
	$sql = "UPDATE users SET identity_check=0 WHERE id={$kaiin_user_id} LIMIT 1";
	$rs = CustomQuery($sql);
}
