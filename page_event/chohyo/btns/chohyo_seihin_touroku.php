...
while($data = $button->getNextSelectedRecord()){
	...
	if($discrimination != ''){
		...
		$discrimination_res = CustomQuery($discrimination_query);
		$discrimination_data = db_fetch_array($discrimination_res);
	...
	if($chohyo_id == ''){
		...
		//商品に帳票IDを反映,ステータスを変更
		if($up_id != ''){
			$sql = "UPDATE shouhin SET `chohyo_type`=1, `status`=133 , chohyo_id = '{$chohyo_id}' , updated_by = {$uid} , updated_at = {$time} WHERE product_id = {$up_id} limit 1";
			// $result["txt"] = $sql;
		}
