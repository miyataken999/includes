...
$shouhin_scan_time = array();// 最初のスキャン時刻
...
while($scan_data = db_fetch_array($scan_rs)){
  ...
  if($scan_data["product_id"] != ""){
    ...
    $shouhin_scan_time[$scan_data["product_id"]] = date("y/m/d H:i:s", strtotime($scan_data["created_at"]));
  }else{
...
// 地金商品登録の時刻によってわけるclassを生成するための変数たち。
$gold_check_add_time_color_i = 0;
$gold_check_add_time__last = "";
...
while($jigane_data = db_fetch_array($jigane_rs)){
  ...
  // if(isset($jigane_data['id'])){
    $haraidashi_query = "SELECT sum(gram) as gram, sum(price) as price from gold_check_item where `gold_check_id`='{$gold_check_id}' AND `name` = '{$jigane_data[id]}'";
  // }
  ...
  while($haraidashi_data = db_fetch_array($haraidashi_rs)){
    $haraidashi_price = $haraidashi_data["price"];
    $haraidashi_gram = $haraidashi_data["gram"];
  }
  $gram_sabun = $haraidashi_gram - $jigane_data['sumGram'];
  $gram_sabun = floor($gram_sabun * 10)/10;
  $price_sabun = $haraidashi_price - $jigane_data["sumPrice"];
  $price_sabun = number_format($price_sabun);
    ...
    $jigane_shouhin_query = "SELECT
      ...
      `A`.`title` AS `title`,
      `A`.`addtime_gold_check_id` AS `addtime_gold_check_id`
      FROM `shouhin` `A`
    ...
    while($jigane_product = db_fetch_array($jigane_shouhin_rs)){
      ...
      $wariai[$jigane_product["product_id"]]["nyuukin_price"] = floor($wariai[$jigane_product["product_id"]]["wariai"] * $sum_gold_check_price / $tax);
      ...
      if($jigane_product["addtime_gold_check_id"] != ""){
        if($gold_check_add_time__last < date("dHis", strtotime($jigane_product["addtime_gold_check_id"]))){
          $gold_check_add_time__last = date("dHis", strtotime($jigane_product["addtime_gold_check_id"])) + 10;
          $gold_check_add_time_color_i++;
        }
        $gold_check_add_time_class = "gold_check_add_time_color_class__" . $gold_check_add_time_color_i;
      }else{
        if(isset($_GET["color_test"])){
          $gold_check_add_time_color_i++;
          $gold_check_add_time_class = "gold_check_add_time_color_class__" . $gold_check_add_time_color_i;
        }else{
          $gold_check_add_time_class = "";
        }
      }
    
      $jigane_table .= "
      <tr class='jigane_table_row {$gold_check_add_time_class}' product_id='{$jigane_product["product_id"]}' >
      ...
      <td class='left_aligned'>{$jigane_product["addtime_gold_check_id"]}</td>
      <td class='{$shouhin_scan[$jigane_product["product_id"]]}'>{$shouhin_scan_time[$jigane_product["product_id"]]}</td>
      </tr>
      ";
...
<table class="error_scan_table only_shouhin_table">
  <tr>
    <th>#</th>
    <th>
      <span><input class="target_jiganeId" type="text" value="{{default_gold_check_id}}"></span>
      <button class="btn btn-success batch_to_jiganeId_btn">一括変更</button>
    </th>
    <th>地金ID</th>
    <th>顧客</th>
    <th>宅配ID</th>
  ...
  <tr>
    <th><button class="btn-danger btn_delete_scan_id">削除</button></th>
    <th>スキャン内容</th>
...
foreach ($only_scan_id as $scan_id) {
  ...
  $only_scan_data_html .= "<tr>";
    ...
    $only_scan_data_html .= "<td><input class='batch_target_seq' type='checkbox' data_type='scan_only' product_id='{$scan_id}'></td>";
    ...
    $only_scan_data_html .= "<td>{$users_array[$scan_data[satei_by]]}<br>{$scan_data[satei_hi]}</td>";
    $only_scan_data_html .= "<td>{$users_array[$scan_data[REG_AUTHOR]]}<br>{$scan_data[DT_UP_DATE]}</td>";
...
foreach ($only_shouhin_id_array as $product_id_data) {
  ...
  $no_scan_data_html .= "<td><input class='batch_target_seq' type='checkbox' product_id='{$product_id_data[product_id]}'></td>";
...
if($only_scan_data_html != '' || $no_scan_data_html != ''){
  ...
  $scan_data_html = str_replace("{{default_gold_check_id}}", $gold_check_id, $scan_data_html);
...
<style>
.gold_check_add_time_color_class__1 td{ background-color: #b5e7e9; }
.gold_check_add_time_color_class__2 td{ background-color: #b5e9ba; }
.gold_check_add_time_color_class__3 td{ background-color: #dde9b5; }
.gold_check_add_time_color_class__4 td{ background-color: #e9deb5; }
.gold_check_add_time_color_class__5 td{ background-color: #e9c9b5; }
.gold_check_add_time_color_class__6 td{ background-color: #e9b5b5; }
.gold_check_add_time_color_class__7 td{ background-color: #e9b5d2; }
.gold_check_add_time_color_class__8 td{ background-color: #c8b5e9; }
.gold_check_add_time_color_class__9 td{ background-color: #b5b8e9; }
.gold_check_add_time_color_class__10 td{ background-color: #dddef4; }
.gold_check_add_time_color_class__11 td{ background-color: #989bf8; color: #000; }
.gold_check_add_time_color_class__12 td{ background-color: #98c3f8; color: #000; }
.gold_check_add_time_color_class__13 td{ background-color: #98f7f8; color: #000; }
.gold_check_add_time_color_class__14 td{ background-color: #98f8bb; color: #000; }
.gold_check_add_time_color_class__15 td{ background-color: #d1f898; color: #000; }
.gold_check_add_time_color_class__16 td{ background-color: #f8de98; color: #000; }
.gold_check_add_time_color_class__17 td{ background-color: #f89898; color: #000; }
.gold_check_add_time_color_class__18 td{ background-color: #f898db; color: #000; }
.gold_check_add_time_color_class__19 td{ background-color: #ca98f8; color: #000; }
.gold_check_add_time_color_class__20 td{ background-color: #989bf8; color: #000; }
...
.jigane_shouhin_row table {
  ...
  button.btn.btn-success.batch_to_jiganeId_btn {
    font-size: 12px;
    padding: 0.3rem;
  }
  input.target_jiganeId.target_jiganeId{
    width: 56px!important;
    min-width: initial!important;
    margin-bottom: 0.5rem;
  }
  #jiganetaiki_table td.scan_ok {
    font-size: 12px;
  }    
</style>
...
$(document).on("click", ".batch_to_jiganeId_btn", function(){
  var seq_txt = ''
  var confirm_txt = ''
  let comfirm_msg
  let target_gold_check_id = $(".target_jiganeId").val()

  let checked_count = $(".batch_target_seq:checked").length
  if(checked_count == 0){
    alert("チェックされたものがありません。。")
    return false;
  }else if(target_gold_check_id == ''){
    alert("変更したい地金チェックIDが入力されていません。。")
    return false;
  }

  $(".batch_target_seq:checked").each(function(i,e){
    ...
    seq_txt += $(e).attr("product_id")
    confirm_txt += $(e).attr("product_id")
  })


  
  comfirm_msg = "以下のSEQを地金チェックID：" + target_gold_check_id + "　に一括変更しますか？\n"
  comfirm_msg += confirm_txt

  let result = confirm(comfirm_msg);
  if (result == true) {
    let batch_url = "http://rifa.life/evaProject/gold_check/batch_update_to_jigane_id?gold_check_id=" + target_gold_check_id + "&seq_txt="+seq_txt
    location.href = batch_url;
  }else{
    alert("バッチ処理をキャンセルしました！")
  }  
})

$(document).on("click", ".btn_delete_scan_id", function(){
  var scan_id_txt = ''
  var confirm_txt = ''
  let comfirm_msg

  if($(".batch_target_seq[data_type='scan_only']:checked").length == 0){
    alert("チェックされたものがありません。。")
    return false;
  }

  $(".batch_target_seq[data_type='scan_only']:checked").each(function(i,e){
    ...
    scan_id_txt += $(e).attr("product_id")
    confirm_txt += $(e).attr("product_id")
  })


  
  comfirm_msg = "以下スキャンのみを削除しますか？\n"
  comfirm_msg += confirm_txt

  let result = confirm(comfirm_msg);
  if (result == true) {
    let batch_url = "http://rifa.life/evaProject/api/gold_check/remove_scan_data?gold_check_id=<?=$gold_check_id?>&scan_id_txt=" + scan_id_txt
    location.href = batch_url;
  }else{
    alert("バッチ処理をキャンセルしました！")
  }  
