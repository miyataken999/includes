...
if(count($only_scan_id) > 0){
?>
<table class="error_scan_table only_scan_table">
  <tr>
    <th colspan="3">スキャンのみにある内容</th>
  </tr>
  <tr>
    <th>スキャン内容</th>
    <th>地金チェックID</th>
...
foreach ($only_scan_id as $scan_id) {
  ...
  $scan_sql = "SELECT gold_check_id, `status`
    FROM shouhin
    WHERE product_id = '{$scan_id}' limit 1
    ";
...
if((count($only_shouhin_id_array) > 0) && (count($scan_all_scan_id_array) > 0)){
?>
<table class="error_scan_table only_shouhin_table">
  <tr>
    <th colspan="14">スキャンされていない商品</th>
  </tr>
  <tr>
    <th>SEQ</th>
    <th>顧客</th>
...
?>
  <tr>
    <td>
      <a href="/shop5/shouhin_list.php?q=(product_id~equals~<?=$product_id_data["product_id"]?>)">
        <?=$product_id_data["product_id"]?>
      </a>
    </td>
    <td><?=$product_id_data["ecc_id"]?></td>
    <td><?=$product_id_data["Eoc_takuhai_id"]?></td>
    <td><?=$product_id_data["Eoc_unfinished_id"]?></td>
    <td><?=$product_id_data["purchase_category"]?></td>
    <td><?=$product_id_data["goods_status"]?></td>
    <td><?=$product_id_data["satei_by"]?></td>
    <td><?=$product_id_data["satei_hi"]?></td>
    <td><?=$product_id_data["REG_AUTHOR"]?></td>
    <td><?=$product_id_data["DT_UP_DATE"]?></td>
    <td><?=$product_id_data["Gram"]?></td>
    <td><?=$product_id_data["price_per_gram"]?></td>
    <td><?=$product_id_data["price"]?></td>
    <td><?=$product_id_data["gold_property"]?></td>
  </tr>
<?php
}// end foreach scan
?>
...
$("#gold_check_btn").click(function(){
  ...
  // console.log($sum_check_price);
  // console.log($total1_price);
