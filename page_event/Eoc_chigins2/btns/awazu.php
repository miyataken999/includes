...
while($data = $button->getNextSelectedRecord()){
	...
	while ($seqresdata = db_fetch_array($seqres))
	{
		...
		$seq_set_query = "UPDATE Eoc_chigins2 SET product_id={$get_product_id} , `status`='34' , updated_by={$uid} , update_at=NOW() WHERE chigin_id={$chigin_id}";
		CustomQuery($seq_set_query);
		// $params["txt"] .= $sql;

		$params["txt"] .= "完了しました！<br>";
		// $_SESSION["Eoc_chigins2_move_page"] = 'awazu';
	}
...
if(count($product_id_list_array) > 0){
	...
	$log_descri = $log_descri."合わずボタンバッチ処理\n";
