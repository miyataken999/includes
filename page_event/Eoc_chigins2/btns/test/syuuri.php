...
while($data = $button->getNextSelectedRecord()){
	...
	$up_values .= ",'{$category_id}'";
	$up_values .= ",'{$sub_category_id1}'";
	$up_values .= ",'{$yahoo_sex}'";
	$up_values .= ",'{$line}'";
	$up_values .= ",'{$item_name}'";
	$up_values .= ",'{$MATERIAL}'";
	...
	while ($seqresdata = db_fetch_array($seqres))
	{
		...
		$seq_set_query = "UPDATE Eoc_chigins2 SET product_id={$get_product_id} , `status`='110' , updated_by={$uid} , update_at=NOW() WHERE chigin_id={$chigin_id}";
		...
		// $_SESSION["Eoc_chigins2_move_page"] = 'kaibutu';
	}
...
if(count($product_id_list_array) > 0){
	...
	$log_descri = $log_descri."修理中ボタンバッチ処理\n";
