...
while($data = $button->getNextSelectedRecord()){
	...
	while ($seqresdata = db_fetch_array($seqres))
	{
		...
		$seq_set_query = "UPDATE Eoc_chigins2 SET product_id={$get_product_id} , `status`='33' , updated_by={$uid} , update_at=NOW() WHERE chigin_id={$chigin_id}";
...
if(count($product_id_list_array) > 0){
	...
	$log_descri = $log_descri."売却済ボタンバッチ処理\n";
