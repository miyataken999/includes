...
while($data = $button->getNextSelectedRecord()){
	...
	// $sql = "UPDATE shouhin SET tentou_label_output_flag = 1 , kaitori_by={$uid} , kaitory_hi = {$time} , updated_by = {$uid} , updated_at = {$time} WHERE product_id = {$up_id}";
	$sql = "UPDATE shouhin SET tentou_label_output_flag = 1 , updated_by = {$uid} , updated_at = {$time} WHERE product_id = {$up_id}";
	CustomQuery($sql);
...
$log_descri = $log_descri."tentou_label_output_flag[new]:1\n";
