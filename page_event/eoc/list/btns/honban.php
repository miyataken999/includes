...
if($Eoc_takuhai_satei_id > 0){
  $html .= "<a class='btn btn-outline-primary' style='color: #28a745;border-color: #28a745;' href='https://rifa.life/evaProject/many_satei/{$Eoc_takuhai_satei_id}?is_direct_edit'>大量査定　直接編集</a>";
  if($c_data["is_many_product"] == 1){
    ...
    if($tairyou_ct > 0){
      $html .= "<a class='btn btn-outline-primary' href='https://rifa.life/evaProject/many_satei/{$Eoc_takuhai_satei_id}'>大量査定 一覧({$tairyou_ct})</a>";
    }else{
      $html .= "<a class='btn btn-outline-primary' href='https://rifa.life/evaProject/many_satei/{$Eoc_takuhai_satei_id}'>大量査定 一覧</a>";
    }
  }else{
    $html .= "<a class='btn btn-outline-primary' href='https://rifa.life/evaProject/many_satei/{$Eoc_takuhai_satei_id}'>大量査定 一覧</a>";
  }
...
$c_sql = "SELECT COUNT(`product_id`) as 'ct' from shouhin where (ecc_id={$ecc_id}) AND (`status` =126)";
$c_rs = CustomQuery($c_sql);
$c_data = db_fetch_array($c_rs);
$ct = $c_data["ct"];
if($ct > 0){	$ct = " ({$ct})";}else{	$ct = "";}
...
(`category_id` =  10 OR `category_id` =  12 OR `category_id` =  17 OR `category_id` =  18 OR `category_id` =  19 OR `category_id` =  50 OR `category_id` =  20 OR `category_id` =  51 OR `category_id` =  21 OR `category_id` =  22 OR `category_id` =  23 OR `shouhin`.`category_id` =  69 OR `shouhin`.`category_id` =  68 OR `shouhin`.`category_id` =  67 OR `shouhin`.`category_id` =  66 OR `shouhin`.`category_id` =  65)
...
$c_sql = "SELECT COUNT(`product_id`) as 'ct' from shouhin where (ecc_id={$ecc_id}) AND
(`category_id` !=  10 AND `category_id` !=  17 AND `category_id` !=  18 AND `category_id` !=  19 AND `category_id` !=  50 AND `category_id` !=  20 AND `category_id` !=  51 AND `category_id` !=  21 AND `category_id` !=  22 AND `category_id` !=  23 AND `category_id` !=  69 AND `category_id` !=  68 AND `category_id` !=  67 AND `category_id` !=  66 AND `category_id` !=  65)
AND (`status` =32 OR `status` =34)";
$c_rs = CustomQuery($c_sql);
$c_data = db_fetch_array($c_rs);
$ct = $c_data["ct"];
...
$html .= "<a class='btn btn-outline-primary' href='shouhin_list.php?q=(ecc_id~equals~{$ecc_id})'>商品一覧{$ct}</a>";
...
if($c_data["ct"] > 0){
  $c_sql = "SELECT `id` FROM `Eoc_unfinished` WHERE `ecc_id` = {$ecc_id} ORDER BY `id` DESC LIMIT 1";
  $c_rs = CustomQuery($c_sql);
  $c_data = db_fetch_array($c_rs);
  $Eoc_unfinished_id = $c_data["id"];
  $html .= "<a class='btn btn-outline-primary' href= 'https://rifa.life/evaProject/many_satei/".$Eoc_unfinished_id."/tentou?is_direct_edit'>【店頭取引】大量査定</a>";
}
...
$html .= "<a class='btn btn-' href='/include/print/refining/deposit.php?ecc_id={$ecc_id}' target='_blank'>分割預かり連絡</a>";
$html .= "<a class='btn btn-' href='/include/print/refining/delivery.php?ecc_id={$ecc_id}' target='_blank'>分割納品連絡</a>";
...
$c_sql = "SELECT COUNT(`id`) as 'ct' from Eoc_refining_v1 where (ecc_id={$ecc_id})";
$c_rs = CustomQuery($c_sql);
$c_data = db_fetch_array($c_rs);
$ct = $c_data["ct"];
if($ct > 0){	$ct = " ({$ct})";}else{	$ct = "";}
$html .= "<a class='btn btn-outline-primary' href='Eoc_refining_v1_list.php?q=(ecc_id~equals~{$ecc_id})'>精錬申し込み{$ct}</a>";
...
$c_sql = "SELECT COUNT(`id`) as 'ct' from updated_Eoc_logs where (ecc_id={$ecc_id})";
$c_rs = CustomQuery($c_sql);
$c_data = db_fetch_array($c_rs);
$ct = $c_data["ct"];
if($ct > 0){	$ct = " ({$ct})";}else{	$ct = "";}
$html .= "<a class='btn btn-outline-primary' href='updated_Eoc_logs_list.php?masterkey1={$ecc_id}&mastertable=Eoc'>店頭更新履歴{$ct}</a>";


$c_sql = "SELECT COUNT(`id`) as 'ct' from ycbm_bookings where (ecc_id={$ecc_id})";
$c_rs = CustomQuery($c_sql);
$c_data = db_fetch_array($c_rs);
$ct = $c_data["ct"];
if($ct > 0){	$ct = " ({$ct})";}else{	$ct = "";}
$html .= "<a class='btn btn-outline-primary' href='ycbm_bookings_list.php?q=(ecc_id~equals~{$ecc_id})'>YCBM店頭予約{$ct}</a>";
