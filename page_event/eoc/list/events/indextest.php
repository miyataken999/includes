		...
		$html .= "<td>";
			...
			$html .= '<table class="event_table table table-bordered table-striped bs-flexgrid rnr-b-grid rnr-gridtable" cellpadding="0">';
				...
				while($data = db_fetch_array($rs)){
					...
					$html .= '<tr class=" bs-gridrow event_'.$data["content"].'">';
					// $html .= '<td class=" bs-gridcell">'.$data["id"];
					...
					if($data["event"] != ''){
						...
						$in_sql = "SELECT `name` FROM mst_Eoc_received_events WHERE id='".$data["event"]."' limit 1";
					...
					if($data["content"] != ''){
						$in_sql = "SELECT `name` FROM mst_Eoc_received_event_contents WHERE id='".$data["content"]."' limit 1";
						$in_rs = CustomQuery($in_sql);
						$in_data = db_fetch_array($in_rs);
						$html .= $in_data["name"];
					}
					...
					$html .= '更新：';
					if($data["update_by"] != ''){
						$in_sql = "SELECT `first_name` FROM users WHERE user_id='".$data["update_by"]."' limit 1";
						$in_rs = CustomQuery($in_sql);
						$in_data = db_fetch_array($in_rs);
						$first_name_array = explode("　",$in_data["first_name"]);
						$html .= $first_name_array[0];
					...
					$html .= ' ';
					if($data["update_at"] != ''){
						$update_at = date("Y",strtotime($data["update_at"])) - 2000 . date("md/Hi",strtotime($data["update_at"]));
