...
while($data = $button->getNextSelectedRecord()){
	...
	$up_product_num = $data["product_num"];
	$sql = "UPDATE shouhin SET product_num = '{$up_product_num}' ,updated_by = {$uid} , updated_at = {$time} WHERE product_id = {$up_id}";
	CustomQuery($sql);
	...
	//-------追加ログ全てのパターン
	$log_text .= "product_num[new]:{$up_product_num}\n";
