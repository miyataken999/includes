...
if($finish!==""){
  ...
  if($mst_Finish_rs!==false){
    ...
    if(($finish_id==1)or($finish_id==12)or($finish_id==10)or($finish_id==11)){
      $finish_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `mst_Finish_id`='$finish_id' AND `category_id`='$category_id' LIMIT 1";
    }elseif($finish_id==2){
      $finish_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `mst_Finish_id`='$finish_id' AND `parent_stone_lower_limit` <= '$parent_stone' ORDER BY `parent_stone_lower_limit` DESC LIMIT 1";
    }else{
      $finish_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `mst_Finish_id`='$finish_id'";
    }

    $finish_cost_rs = CustomQuery($finish_cost_query);
    if($finish_cost_rs!==false){
      $finish_cost_data = db_fetch_array($finish_cost_rs);
      $finish_cost = $finish_cost_data["cost"];//mst参照　鑑別コスト
    }else{
      $finish_cost = "";
    }
  }
...
//   // $values["comment"] = $shouhin_finish_query;
    ...
    if($finish_cost_rs!=false){
      ...
      while($finish_cost_data = db_fetch_array($finish_cost_rs)){
        ...
        if($all_ok == 'ok'){
          ...
          break;
          // $loop_flag = 'break';
          // for ($last_i=0; $last_i < count($last_column_array); $last_i++) {
          //   $last_column = $last_column_array[$last_i];
          //   if($discrim_cost_data[$last_column] != ''){
          //     $loop_flag = '';
          //   }
          // }
          // if($loop_flag == 'break'){
...
if("kanbetsu"=="old"){
  ...
  // if($discrimination!==""){
  //   $mst_Discrim_query = "SELECT `cost` FROM `mst_Discrimination` WHERE `val`=\"$discrimination\" LIMIT 1";
  //   $mst_Discrim_rs = CustomQuery($mst_Discrim_query);
  //   if($mst_Discrim_rs!==false){
  //     $mst_Discrim_data = db_fetch_array($mst_Discrim_rs);
  //     $discrimination_cost = $mst_Discrim_data["cost"];//mst参照　鑑別コスト
  //   }
  // }
  // if(isset($discrimination_cost)){
  ...
  //   // $shouhin_discrimination_rs = CustomQuery($shouhin_discrimination_query);
  // }
  //
  // $sub_cate_query = "SELECT * FROM `store_sub_categories` WHERE `id`={$values['sub_category_id1']} LIMIT 1";
  // $sub_cate_rs = CustomQuery($sub_cate_query);
  // $sub_cate_data = db_fetch_array($sub_cate_rs);
  // $colorstone_id = $sub_cate_data["colorstone_id"];
  // // $values["colorstone_id"] = $colorstone_id;
  //
  // $DA_CUT_query = "SELECT * FROM `shouhin` WHERE `product_id`={$product_id} LIMIT 1";
  // $DA_CUT_rs = CustomQuery($DA_CUT_query);
  // $DA_CUT_data = db_fetch_array($DA_CUT_rs);
  // $DA_CUT = $DA_CUT_data["DA_CUT"];
  // $DIA_DA_CUT_query = "SELECT `id` FROM `_DIA_DA_CUT` WHERE `name`=\"$DA_CUT\" LIMIT 1";
  // $DIA_DA_CUT_rs = CustomQuery($DIA_DA_CUT_query);
  // $DIA_DA_CUT_data = db_fetch_array($DIA_DA_CUT_rs);
  // // $values["_DIA_DA_CUT_id"] = $DIA_DA_CUT_data["id"];
  // $values["comment"] = $shouhin_discrimination_query;
  // if($discrimination_id==11){//C通鑑
  //   $discrim_cost_query = "SELECT `cost`,`category_id`,`colorstone_id` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id ORDER BY `colorstone_id` DESC";
  ...
  //       if($discrim_cost_data["colorstone_id"]==$colorstone_id){
  //         break;
  //       }
  //     }
  //   }
  // }elseif($discrimination_id==12){//C通鑑/産特
  //   $discrim_cost_query = "SELECT `cost`,`category_id`,`colorstone_id` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id ORDER BY `colorstone_id` DESC";
  //   $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //   if($discrim_cost_rs!=false){
  //     while($discrim_cost_data = db_fetch_array($discrim_cost_rs)){
  //       if(($discrim_cost_data["colorstone_id"]==$colorstone_id)and($discrim_cost_data["category_id"])==$category_id){
  //         $discrimination_cost = $discrim_cost_data["cost"];
  //         break;
  //       }
  //     }
  //   }
  // }elseif(($discrimination_id==5)or($discrimination_id==14)or($discrimination_id==15)or($discrimination_id==23)){
  //   //GIA/GGO、GIA/DD、GIA/DGR、GIA/GN
  //   $discrim_cost_query = "SELECT `cost`,`effect` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id AND `colorstone_id`=$colorstone_id AND `parent_stone_lower_limit` <= $parent_stone ORDER BY `parent_stone_lower_limit` DESC LIMIT 1";
  //   $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //   $discrim_cost_data = db_fetch_array($discrim_cost_rs);
  //   if($discrim_cost_data!=false){
  //     if($discrim_cost_data["effect"]!=""){
  //       if($values['effect']==$discrim_cost_data["effect"]){
  //         $discrimination_cost = $discrim_cost_data["cost"];
  //       }
  //     }else{
  //       $discrimination_cost = $discrim_cost_data["cost"];
  //     }
  //   }
  // }elseif($discrimination_id==1){//Cソ
  //   if($colorstone_id==1){
  //     $discrim_cost_query = "SELECT `shape`,`cutting_syle`,`cost` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id AND `colorstone_id`=$colorstone_id AND `parent_stone_lower_limit` <= $parent_stone ORDER BY `parent_stone_lower_limit` DESC,`shape` DESC,`cutting_syle` DESC LIMIT 2";
  //     $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //     if($discrim_cost_rs!=false){
  //       while($discrim_cost_data = db_fetch_array($discrim_cost_rs)){
  //         $discrimination_cost = $discrim_cost_data["cost"];
  //         if(($values['shape']==$discrim_cost_data['shape'])and($values['cutting_style']==$discrim_cost_data['cutting_syle'])){
  //           break;
  //         }//if
  //       }//while
  //     }//if
  ...
  //       $no_target_cut = "on";
  //     }else{
  //       $discrim_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id AND `DA_CUT`= $DA_CUT_id AND `status`=$_st AND `parent_stone_lower_limit` <= $parent_stone ORDER BY `parent_stone_lower_limit` DESC LIMIT 1";
  //       $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //       if($discrim_cost_rs!=false){
  //         $discrim_cost_data = db_fetch_array($discrim_cost_rs);
  //         $discrimination_cost = $discrim_cost_data["cost"];
  ...
  //     }
  //     if($no_target_cut == "on"){
  //       $discrim_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id AND ISNULL(`DA_CUT`) AND `status`=$_st AND `parent_stone_lower_limit` <= $parent_stone ORDER BY `parent_stone_lower_limit` DESC LIMIT 1";
  //       $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //       $discrim_cost_data = db_fetch_array($discrim_cost_rs);
  //       $discrimination_cost = $discrim_cost_data["cost"];
  //     }
  //   }//if($colorstone_id==1)
  // }elseif($discrimination_id==2){//Cダ
  //   $discrim_cost_query = "SELECT `shape`,`cutting_syle`,`cost` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id AND `colorstone_id`=$colorstone_id AND `parent_stone_lower_limit` <= $parent_stone ORDER BY `parent_stone_lower_limit` DESC,`shape` DESC,`cutting_syle` DESC LIMIT 2";
  //   $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //   if($discrim_cost_rs!=false){
  //     while($discrim_cost_data = db_fetch_array($discrim_cost_rs)){
  ...
  //   }//if
  // }else{
  //   $discrim_cost_query = "SELECT `cost` FROM `mst_finish_terms` WHERE `Discrimination`=$discrimination_id LIMIT 1";
  //   $discrim_cost_rs = CustomQuery($discrim_cost_query);
  //   if($discrim_cost_rs!=false){
  //     $discrim_cost_data = db_fetch_array($discrim_cost_rs);
  //     $discrimination_cost = $discrim_cost_data["cost"];
...
if($discrimination!==""){
  ...
  if($mst_Discrim_rs!==false){
    ...
    if($discrim_cost_rs!=false){
      $last_column_array = array();
      while($discrim_cost_data = db_fetch_array($discrim_cost_rs)){
        // $discrimination_cost .= $discrim_cost_data["colorstone_id"];
        $all_ok = 'ok';
        // $value_count = 0;
        // $value_match_count = 0;
        for ($rule_i=0; $rule_i < count($rule_array); $rule_i++) {
      ...
      // $values["comment"] = "UPDATE `shouhin` SET `identification_cost` = $discrimination_cost WHERE `product_id` = {$product_id} LIMIT 1";
