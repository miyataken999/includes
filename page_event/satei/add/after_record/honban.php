...
if(($new_st == 32) OR ($new_st == 33) OR ($new_st == 126)){
  ...
  //服査の場合は複数枚は不要。1枚だけにする。
  $current_page_pass = $_SERVER["HTTP_REFERER"];
  if(stristr($current_page_pass, "satei_of_brand1_list")){
  	if($label_output_count > 1){
      $label_output_count = 1;
    }
  }
  ...
  if($label_data == NULL){
    $user_label_output_query = "INSERT INTO user_label_output (`user_id`, `product_id`, `create_at`, `type`, `number_of_sheets`, `output`)VALUES('{$user_id}', '{$product_id}', NOW(), '{$label_tmp}', '{$label_output_count}', '{$output}')";
