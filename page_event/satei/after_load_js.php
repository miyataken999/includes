...
if(stristr($HTTP_REFERER, "satei_of_jewelry_list")){
	$hanbai_ritsu = 0.5;
}else{
	$hanbai_ritsu = 0.4;
}
...
?>
<table id="kataban_price_area">
	<tr>
		<td colspan="4">過去の買取額</td>
	</tr>
	<tr id="kataban_price_area_brand">
		<td colspan="4" id="kataban_price_box_title"></td>
	</tr>
	<tr>
		<th></th>
		<th>最小値</th>
		<th>最大値</th>
		<th class="average">平均値</th>
	</tr>
	<tr>
		<th class="condition_th_N">N</th>
		<th id="N_min_price"></th>
		<th id="N_max_price"></th>
		<th class="average" id="N_avg_price"></th>
	</tr>
	<tr>
		<th class="condition_th_S">S</th>
		<th id="S_min_price"></th>
		<th id="S_max_price"></th>
		<th class="average" id="S_avg_price"></th>
	</tr>
	<tr>
		<th class="condition_th_A">A</th>
		<th id="A_min_price"></th>
		<th id="A_max_price"></th>
		<th class="average" id="A_avg_price"></th>
	</tr>
	<tr>
		<th class="condition_th_B">B</th>
		<th id="B_min_price"></th>
		<th id="B_max_price"></th>
		<th class="average" id="B_avg_price"></th>
	</tr>
	<tr>
		<th class="condition_th_C">C</th>
		<th id="C_min_price"></th>
		<th id="C_max_price"></th>
		<th class="average" id="C_avg_price"></th>
	</tr>
	<tr>
		<th class="condition_th_X">X</th>
		<th id="X_min_price"></th>
		<th id="X_max_price"></th>
		<th class="average" id="X_avg_price"></th>
	</tr>
	<!-- <tr>
		<td id="kataban_price_box_min"></td>
		<td id="kataban_price_box_max"></td>
		<td id="kataban_price_box_avg"></td>
	</tr> -->
</table>
<style type="text/css">
	#kataban_price_area.active{
		display: block;
	}
	#kataban_price_area_brand {
...
function kataban_price(openflag,input_brand,input_kataban){
	if(openflag == 'on'){
		$("#kataban_price_area").addClass('active');
		// $("#kataban_price_box_title").html(input_brand+"<br>"+input_kataban);

		$.ajax({
			url: 'https://rifa.life/include/api/kataban_price.php',
			type: 'GET',
			dataType: 'json',
			data: {
				brand: input_brand,
				kataban: input_kataban,
			}
		})
		.done(function(data) {
			console.log("success");
			console.log(data);
			// $explode_data = getValue(data);
			// console.log($explode_data);
			...
			// アイテムのコンディションごとの最小、大、平均値

			$("#kataban_price_box_title").html(data.brand[0].brand_name+"<br>"+input_kataban);


			$("#N_min_price").html(data.price[0].N_min_price);
			$("#N_avg_price").html(data.price[0].N_avg_price);
			$("#N_max_price").html(data.price[0].N_max_price);
			$("#S_min_price").html(data.price[0].S_min_price);
			$("#S_avg_price").html(data.price[0].S_avg_price);
			$("#S_max_price").html(data.price[0].S_max_price);
			$("#A_min_price").html(data.price[0].A_min_price);
			$("#A_avg_price").html(data.price[0].A_avg_price);
			$("#A_max_price").html(data.price[0].A_max_price);
			$("#B_min_price").html(data.price[0].B_min_price);
			$("#B_avg_price").html(data.price[0].B_avg_price);
			$("#B_max_price").html(data.price[0].B_max_price);
			$("#C_min_price").html(data.price[0].C_min_price);
			$("#C_avg_price").html(data.price[0].C_avg_price);
			$("#C_max_price").html(data.price[0].C_max_price);
			$("#X_min_price").html(data.price[0].X_min_price);
			$("#X_avg_price").html(data.price[0].X_avg_price);
			$("#X_max_price").html(data.price[0].X_max_price);
			// $("#kataban_price_box_min").html(data[0][0].min_price);
			// $("#kataban_price_box_avg").html(data[0][0].avg_price);
			// $("#kataban_price_box_max").html(data[0][0].max_price);
			// $("#satei_hi_min").html(data.min[0].satei_hi);
			// $("#yahoo_condition2_min").html(data.min[0].yahoo_condition2);
			// $("#satei_hi_max").html(data.max[0].satei_hi);
			// $("#yahoo_condition2_max").html(data.max[0].yahoo_condition2);
		...
		$("#kataban_price_area").removeClass('active');
		$("#kataban_price_box_title").html('');
		$("#kataban_price_box_min").html('');
		$("#kataban_price_box_avg").html('');
		$("#kataban_price_box_max").html('');
...
//====================================================================
function js_load(){
	$("[id*=value_yahoo_kataban]").focusout(function(){
		...
		$kataban = $(this).val();
		if(($kataban == '') || ($value_product_num == '')){
			kataban_price('','',$kataban);
		}else{
			kataban_price('on',$value_product_num,$kataban);
		}
	});
...
<?
//宝査のみの項目。服査ではundifinedになるので、URLで区分けした。20200310 nishitani
if(stristr($HTTP_REFERER, "satei_of_jewelry_list")){
?>
...
?>
	...
	$("[id*=value_yahoo_condition1_]").focusin(function(){
	  //idの固有連番を取得、抜き出す
	  $this_id_num = $(this).attr("id");
	  $this_id_num = $this_id_num.split("_");
	  $this_id_num = $this_id_num[$this_id_num.length-1];
	  ...
	  $category_id = $("#value_category_id_"+$this_id_num).val();

	  var req = new XMLHttpRequest();
	  var result = $("#value_yahoo_condition1_"+$this_id_num);
	  // result.innerHTML = "通信中...";

	  if(result.val() == ''){
	    req.open('GET', 'https://rifa.life/evaProject/dataGet/get_condition_detail/' + $category_id);
	    req.send(null);
	    req.onreadystatechange = function() {
	      if (req.readyState == 4) { // 通信の完了時
	        if (req.status == 200) { // 通信の成功時
	          ...
	          console.log(data);
	          var res = data.condition_details;

	          result.val(res);
	        }
	      }else{
	        // result.innerHTML = "通信に失敗しました。";
	      }
	    }
	  }// end result val
	});
...
function price_keisan(id_num){
	...
	$_sales_price_res = $_price_res/<?=$hanbai_ritsu?>;
...
function hanbaikeisan(id_num){
	...
	$_sales_price_res = $_price/<?=$hanbai_ritsu?>;
	$_sales_price_res = Math.floor($_sales_price_res);
	$("#value_sales_price_"+id_num).val($_sales_price_res);
}
