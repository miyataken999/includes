...
if($gold_check_id == ''){
  ...
  $token = 'tamiya';
  exec("curl -X POST -d 'cw_body={$word}&room={$room}&token={$token}' 'http://rifa.life/lounge_API/chatwork.php' -k");
  exit;
}
...
// var_dump($slip_type);
if($slip_type==1){
  $sql = "UPDATE shouhin SET `gold_check_id`= '{$gold_check_id}',`updated_at` = '{$date}',`updated_by` = '{$uid}' WHERE product_id = ? AND (gold_check_id is NULL OR gold_check_id = '') LIMIT 1";

}elseif($slip_type==2){
  $sql = "UPDATE shouhin SET `status` = 148, `gold_check_id`= '{$gold_check_id}',`updated_at` = '{$date}',`updated_by` = '{$uid}' WHERE product_id = ? AND (gold_check_id is NULL OR gold_check_id = '') LIMIT 1";
...
//view参照
$view_sql = "SELECT * FROM vw_jigane_taiki where slip_type=? AND `status`=119";
...
foreach($view_stmt as $item){
  ...
  $target_sql = "SELECT `gold_property` FROM `shouhin` WHERE `product_id` = $product_id";
...
// $url = "https://urlounge.co.jp/shop5/gold_check_item_list.php?masterkey1=$gold_check_id&mastertable=gold_check";
// header("location:$url");
