...
//地金チェックに完了変更日を入れる
$jigane_status_up_sql = "UPDATE gold_check SET `kanryou_henbi`= '{$kanryou_henbi}', `update_at` = '{$date}', `update_by` = '{$uid}' WHERE id = '{$gold_check_id}' LIMIT 1";
$pdo_jigane_status_up = new pdo_connect;
$jigane_status_up_stmt = $pdo_jigane_status_up->pdo()->prepare($jigane_status_up_sql);
$jigane_status_up_stmt->execute();
...
foreach($view_stmt as $item){
  ...
  $sql = "UPDATE shouhin SET `status`= 142, `nyuukin_price`='{$nyuukin_price}', `kanryou_henbi`= '{$kanryou_henbi}', `updated_at` = '{$date}', `updated_by` = '{$uid}' WHERE product_id = ? LIMIT 1";
  // テストコメント
