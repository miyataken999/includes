...
if($kokyaku != ''){
	...
	if($Eoc_takuhai_id != ''){
		//NEW 金査と査物 同時取得
		$syouryou_query = "SELECT product_num,category_id,price,satei_by,kinsa_flag,purchase_category FROM shouhin WHERE ecc_id={$kokyaku} AND Eoc_takuhai_id={$Eoc_takuhai_id} AND ((`status`!=126) AND (`status`!=32) AND (`status`!=34)) order by product_id";
		// print "syouryou_query {$syouryou_query}";
		// exit();
		...
		while ($syouryou_res = mysql_fetch_assoc($syouryou_sql)) {
			$satei_by = $syouryou_res["satei_by"];
			$syouryou_count++;
			if(
