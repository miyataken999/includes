...
Runner.pages.PageSettings.addPageEvent('PMT_EV001', Runner.pages.constants.PAGE_LIST, "afterPageReady", function( pageObj, proxy, pageid ) {
	//register a new button
	pageObj.buttonNames[ pageObj.buttonNames.length ] = '___4';
	
	if ( !pageObj.buttonEventBefore['___4'] ) {
		pageObj.buttonEventBefore['___4'] = function( params, ctrl, pageObj, proxy, pageid, rowData, row ) {		
// Put your code here.
params["txt"] = "Hello";
ctrl.setMessage("Sending request to server...");
 ...
 // return false;
	...
	if ( !pageObj.buttonEventAfter['___4'] ) {
		pageObj.buttonEventAfter['___4'] = function( result, ctrl, pageObj, proxy, pageid, rowData, row ) {
// Put your code here.
...
ctrl.setMessage(message);
	...
	$('a[id="___4"]').each( function() {
		if ( $(this).closest('.gridRowAdd').length ) {
			return;
		}
		
		this.id = "___4" + "_" + Runner.genId();
		
		// create object
		var button____4 = new Runner.form.Button({
			id: this.id ,
			btnName: "___4"
		});
		
		// init
		button____4.init( {args: [ pageObj, proxy, pageid ]} );
	});
});
...
Runner.pages.PageSettings.addPageEvent('PMT_EV001', Runner.pages.constants.PAGE_EDIT, "afterPageReady", function(pageObj, proxy, pageid, inlineRow, inlineObject) {
...
Runner.pages.PageSettings.addPageEvent('PMT_EV001', Runner.pages.constants.PAGE_ADD, "afterPageReady", function(pageObj, proxy, pageid, inlineRow, inlineObject) {
	$("[id*='value_DA_WEIGHT_'] , [id*='value_DA_THESPECIFICATIONSLOSSES']").keyup(function(){
		//idの固有連番を取得、抜き出す
		$this_id = $(this).attr("id");
		$this_id = $this_id.split("_");
