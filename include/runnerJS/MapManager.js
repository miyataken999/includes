...
Runner.controls.MapManager = Runner.extend( Runner.emptyFn, {
	...
	/**
	 * If it's set to false the markers won't have any click event handlers
	 * @type {boolean} 
	 */
	markerAsLink: true,
	...
	addMarkerClickEvent: function( marker, markerData, markerAsEditLink ) { 
		...
		if ( !markerData.link || !this.markerAsLink ) { 
			return;
		}
		...
		if ( this.showViewInPopup ) {
			...
			this.providerMap.addOnMarkerClickHandler( marker, function() { 				
				var inlineObj = mapManager.pageObj.inlineEdit || mapManager.pageObj.inlineAdd || null,
					pageParams = {
						tName: mapManager.tName, 
						pageType: Runner.pages.constants.PAGE_VIEW, 
						keys: markerData.keys,
						modal: true,
						dashGridBased: inlineObj && inlineObj.dashGridBased,
						baseParams: {
							parId: mapManager.id,
							table: escape( mapManager.tName )
						},
						inlineObjectParams: {
							getInlineObject: function() {
								return inlineObj;
							},
							rowId: markerData.recId
						}
				};
