...
class oLocking
{
	var $lockTableName = "evav62122_locking";
	var $ConfirmTime=250;
	var $UnlockTime=300;
	var $ConfirmAdmin;
	var $ConfirmUser;
	...
	function __construct()
	{
		...
		$this->ConfirmAdmin = "アドミニストレータ「 %s 」が貴方の編集セッションを中止しました。";
		$this->ConfirmUser = "編集セッションがタイムアウトしました。";
		$this->LockAdmin = "レコードは、%s によって%s 分間の間編集されています。";
		$this->LockUser = "レコードは他のユーザにより編集されています。";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."レコードのロック解除".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."レコードの編集".'</a>';
			}
			return $str;
		}
