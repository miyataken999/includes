...
//カテゴリ2以降を取得できるように設定
//+++++++++++++++++++++++++++++++++++++++
$field_name .= " , store_products.product_id , store_products.raku_hyoujisaki_category2 , store_products.raku_hyoujisaki_category3 , store_products.raku_hyoujisaki_category4 , store_products.raku_hyoujisaki_category5 ";
...
//該当ファイルオープン
$fp = fopen('./rakuten/ritem/batch/item-cat.csv','w');
if ($fp === FALSE) {
...
$result_array = array();
	while ($result = mysql_fetch_assoc($sql)) {
		for ($field_i=0; $field_i < $count_field; $field_i++) {
			...
			$category_2 = $result["raku_hyoujisaki_category2"];
			$category_3 = $result["raku_hyoujisaki_category3"];
			$category_4 = $result["raku_hyoujisaki_category4"];
			$category_5 = $result["raku_hyoujisaki_category5"];
			array_push($result_list , $result_array);
			if($category_2 != ""){
				$result_array[3] = $category_2;
				array_push($result_list , $result_array);
				if($category_3 != ""){
					$result_array[3] = $category_3;
					array_push($result_list , $result_array);
					if($category_4 != ""){
						$result_array[3] = $category_4;
						array_push($result_list , $result_array);
						if($category_5 != ""){
							$result_array[3] = $category_5;
							array_push($result_list , $result_array);
						}
					}
				}
...
foreach($result_list as $result_set){
  ...
  if(fputcsv($fp, $result_set)){
  	print("item-cat.csv{$fput_i}行目完了<br>");
  }else{
  	print("item-cat.csv{$fput_i}行目書き込み失敗しました。<br>");
  	exit();
  }
  $fput_i++;
}
