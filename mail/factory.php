...
if($tmp_id != ''){
	...
	if(isset($_GET["test"])){
		$templete_result = $templete_pdo->select_one("SELECT mail_data_tbl_template_test.*,shomei_tbl.str as `shomei` from mail_data_tbl_template_test left outer join shomei_tbl on mail_data_tbl_template_test.str3_pc=shomei_tbl.id where mail_data_tbl_template_test.id={$tmp_id} limit 1");
	}else{
...
if($ecc_id != ''){
	...
	$replace_data["Eoc_takuhai"] = $Eoc_takuhai_pdo->select_one("SELECT * from Eoc_takuhai where ecc_id={$ecc_id} AND (Eoc_takuhai.`status`=1 OR Eoc_takuhai.`status`=12 OR Eoc_takuhai.`status`=5 OR Eoc_takuhai.`status`=6 ) limit 1");
