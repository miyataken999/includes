...
<?
	...
	while ($schema_result = mysql_fetch_assoc($sql)) {
		...
		if(!in_array($schema_result['COLUMN_NAME'], $column_name, true)){
			array_push($column_name, $schema_result['COLUMN_NAME']);
		}
		
		if(!in_array($schema_result['COLUMN_COMMENT'], $column_comment, true)){
...
<div id="sousin_name_box">
	...
	<form action="/include/kagoya_check/takuhai/eoc_after_data_table/eoc_after_data_table_2023test.php" method="get" accept-charset="utf-8">
		<input type="hidden" name="hikaku_id" value="<?=$hikaku_id?>">
