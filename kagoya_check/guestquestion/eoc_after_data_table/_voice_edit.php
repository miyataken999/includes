...
$timestamp = date("Y年m月d日 H:i",strtotime($timestamp));
...
if($voice_sub_id != ""){
	$h1 = "voice編集";
	include $_SERVER["DOCUMENT_ROOT"]."/include/connection.php";
	$query = "SELECT * FROM voice_sub_testtest WHERE id={$voice_sub_id} LIMIT 1";
	$sql = mysql_query('SET NAMES utf8');
	$sql = mysql_query($query);
	...
	$subject .= "<th>満足度</th>";
		...
		//満足度の顔アイコンの設定
		if($service_satisfied == "とても満足"){
			$satisfied_img = "/new/img/guestquestion/section_2/satisfaction1.png";
		}elseif($service_satisfied == "やや満足"){
			$satisfied_img = "/new/img/guestquestion/section_2/satisfaction2.png";
		}elseif($service_satisfied == "普通"){
			$satisfied_img = "/new/img/guestquestion/section_2/satisfaction3.png";
		}elseif($service_satisfied == "やや不満"){
			$satisfied_img = "/new/img/guestquestion/section_2/satisfaction4.png";
		}elseif($service_satisfied == "不満"){
			$satisfied_img = "/new/img/guestquestion/section_2/satisfaction5.png";
		}

	$subject .= "<th>買取種類</th>";
	$subject .= "</tr>";
	$subject .= "<tr>";
	$subject .= "<td><img src='https://brandkaimasu.com{$satisfied_img}' id='satisfied_img_id'>{$service_satisfied}</td>";
	...
	if(($service_speed != "") OR ($service_description != "")){
		$subject .= "<tr>";
		$subject .= "<th>スピード</th>";
		$subject .= "<th>説明</th>";
		$subject .= "</tr>";
		...
		$subject .= "</td>";
		$subject .= "<td>{$service_description}";
		if($service_description_text != ""){
			$subject .= "<br>{$service_description_text}";
		}
		$subject .= "</td>";
		$subject .= "</tr>";
	}
...
<div id="form_back">
	<form id="main_form" action="_voice_edit_submit.php" method="POST">
		<h1><?=$h1?></h1>
