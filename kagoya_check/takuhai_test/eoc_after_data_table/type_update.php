...
$kizon_column = array();
	...
	array_push($kizon_column,'kit_flag');
...
$kizon_query = "SELECT {$kizon_select} FROM {$target_table} WHERE ecc_id={$kousin_id} LIMIT 1";
	...
	while ($kizon_result = mysql_fetch_assoc($kizon_sql)) {
		...
		$kit_flag = $kizon_result['kit_flag'];
...
<?
$kizon_no_space_name = $kizon_name1_sei.$kizon_name1_mei;
$no_space_name = $user_name_sei.$user_name_mei;
if($kizon_no_space_name != $no_space_name){
	$mismatch = "mismatch";
	$mismatch_word ="名前違うよ！";
}else{
	$mismatch = "";
...
////////////////////end【名】文字化け確認//////////////////////
?>
	<tr class="<?=$mismatch?>"><th></th><td colspan="3"><span><?=$mismatch_word?></span></td></tr>
  <?
  if($sei_garbled_alert_flag == "on"){
  ?>
    <td class="alert_row" colspan="3">文字化けするので、旧字体の修正をお願いしますしん！<td>
  <?
  }
  ?>
	<tr class="ecc_row">
		<?$eoc_column_name_i++;?>
		<th class="th_<?=$eoc_up_col[$eoc_column_name_i]['name']?> td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><?=$eoc_up_col[$eoc_column_name_i]['wamei']?></span></th>
		<td class="<?=$alert_row?>kizon_data_td"><span><?=$user_name_sei?></span></td>
		...
		<td class="<?=$alert_row?>td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><input type="text" name="user_name_sei" value="<?=$kizon_name1_sei?>" ></span></td>
	</tr>
  <?
  if($sei_garbled_alert_flag == "on"){
  ?>
  ...
  ?>
	<tr class="ecc_row">
		<?$eoc_column_name_i++;?>
		<th class="th_<?=$eoc_up_col[$eoc_column_name_i]['name']?> td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><?=$eoc_up_col[$eoc_column_name_i]['wamei']?></span></th>
		<td class="<?=$alert_row?> kizon_data_td"><span><?=$user_name_mei?></span></td>
		<td class="copy_col"><span class="<?=$eoc_up_col[$eoc_column_name_i]['name']?>_copy copy_btn" data="<?=$eoc_up_col[$eoc_column_name_i]['name']?>">⇒</span></td>
		<td class="<?=$alert_row?> td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><input type="text" name="user_name_mei" value="<?=$kizon_name1_mei?>" ></span></td>
	</tr>


<?
$kizon_no_space_name = $kizon_name2_sei.$kizon_name2_mei;
$no_space_name = $user_name_kana_sei.$user_name_kana_mei;
if($kizon_no_space_name != $no_space_name){
	$mismatch = "mismatch";
	$mismatch_word ="名前違うよ！";
}else{
...
<?
$trim_kizon_tel = str_replace("-", "", $kizon_tel);
$trim_kizon_tel2 = str_replace("-", "", $kizon_tel2);
$trim_tel_res = str_replace("-", "", $tel_res);
$trim_tel2 = str_replace("-", "", $tel2);

$mismatch = "";
$mismatch_word = "";
if(($trim_kizon_tel != $trim_tel_res) AND ($trim_kizon_tel != $trim_tel2)){
	$mismatch = "mismatch";
	$mismatch_word .= "{$trim_kizon_tel}　今回入力されてません。";
}

?>
	<tr class="<?=$mismatch?>"><th></th><td colspan="2"></td><td><span><?=$mismatch_word?></span></td></tr>
...
$mismatch = "";
$mismatch_word = "";
if(($trim_kizon_tel2 != $trim_tel_res) AND ($trim_kizon_tel2 != $trim_tel2)){
	$mismatch = "mismatch";
	$mismatch_word .= "{$trim_kizon_tel2}　今回入力されてません。";
}
?>
	<tr class="<?=$mismatch?>"><th></th><td colspan="2"></td><td><span><?=$mismatch_word?></span></td></tr>
...
$mismatch = "";
$mismatch_word = "";
if(($user_mail != $kizon_mail1) AND ($user_mail != $kizon_mail2)){
	$mismatch = "mismatch";
	$mismatch_word .= "メアド違うよ！";
}
...
?>
	<tr class="ecc_row <?=$mismatch?>">
		<?$eoc_column_name_i++;?>
		<th class="th_<?=$eoc_up_col[$eoc_column_name_i]['name']?> td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><?=$eoc_up_col[$eoc_column_name_i]['wamei']?></span></th>
		<td class="kizon_data_td"><?=$mismatch_word?><span><?=$user_todou?></span></td>
		<td class="copy_col"><span class="<?=$eoc_up_col[$eoc_column_name_i]['name']?>_copy copy_btn" data="<?=$eoc_up_col[$eoc_column_name_i]['name']?>">⇒</span></td>
		<td class="td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><input type="text" name="user_todou" value="<?=$kizon_address1?>" ></span></td>
	</tr>
<?
if($kizon_address2 != $user_sikutyouson){
	$mismatch = "mismatch";
	$mismatch_word ="違うよ！";
}else{
	$mismatch = "";
...
?>
	<tr class="ecc_row <?=$mismatch?>">
		<?$eoc_column_name_i++;?>
		<th class="th_<?=$eoc_up_col[$eoc_column_name_i]['name']?> td_<?=$eoc_up_col[$eoc_column_name_i]['name']?>"><span><?=$eoc_up_col[$eoc_column_name_i]['wamei']?></span></th>
		<td class="kizon_data_td"><?=$mismatch_word?><span><?=$user_banti?></span></td>
		<td class="copy_col"><span class="<?=$eoc_up_col[$eoc_column_name_i]['name']?>_copy copy_btn" data="<?=$eoc_up_col[$eoc_column_name_i]['name']?>">⇒</span></td>
