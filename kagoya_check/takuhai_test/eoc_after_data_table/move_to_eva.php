...
include $_SERVER["DOCUMENT_ROOT"]."/include/connection_kagoya.php";
	...
	$query = 'SELECT * FROM internet.Eoc_takuhai_copy_tamiya WHERE id='.$move_id.' LIMIT 1';
...
$query = "INSERT INTO internet.Eoc_takuhai_copy200515{$into_col} VALUES{$into_val}";
$sql = mysql_query('SET NAMES utf8');
$sql = mysql_query($query);
$eoc_takuhai_last_id = mysql_insert_id();
	...
	if (!$sql) {
	    ...
	    $c = system('curl -X POST -H "X-ChatWorkToken:'.$token.'" -d "body='.$text_table.'" "https://api.chatwork.com/v2/rooms/'.$room.'/messages" -O',$c);

	    ?>
	    <h2>失敗しました。チャットワークに流します。</h2>
	    <a href='https://www.chatwork.com/#!rid<?=$room?>'>メッセージへ移動</a>
	    <div>詳細</div>
	    <div>
			<?=$text_table?>
	    </div>
...
if(($delete_id == "") AND ($move_id != "")){
	$delete_id = $move_id;
	include_once $_SERVER["DOCUMENT_ROOT"]."/include/kagoya_check/takuhai_test/delete.php";
}
?>
		<h2>成功しました。</h2>
		...
		<script type="text/javascript">
			window.location.href = "/include/kagoya_check/takuhai_test/list.php";
