...
$table_name = "Eoc_takuhai_copy_tamiya";
...
<?
	...
	// $query = 'SELECT COLUMN_NAME , COLUMN_COMMENT FROM information_schema.COLUMNS WHERE TABLE_NAME=`Eoc_takuhai` ORDER BY ORDINAL_POSITION';
...
<?
if($result["ecc_id"] >0 ){
	$ecc_link = "http://urlounge.co.jp/include/kagoya_check/takuhai_test/eoc_after_data_table/eoc_after_data_table.php?hikaku_id={$result['id']}&uid={$uid}&kousin_id={$result['ecc_id']}";
...
?>
		<td class="td_<?=$column_name[$column_i]?> col_<?=$column_name[$column_i]?>"><span><a href='/include/kagoya_check/takuhai_test/hikaku_list.php?hikaku_id=<?=$column_val?>' >対象発掘</a></span></td>
<?
}
?>
