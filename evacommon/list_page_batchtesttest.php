...
if($_POST["process_content"] == "chohyo_touroku_seihin"){
	...
	for ($product_id_array_i=0; $product_id_array_i < count($product_id_array); $product_id_array_i++) {
		...
		if($chohyo_id != ''){
			...
			$word = "[info][title](cracker)帳票id {$chohyo_id}(cracker)[/title]製品：{$id_count}件 ";
			...
			$word = "[info][title](devil)帳票登録エラー(devil)[/title]SEQ：{$up_id}[/info]";
	...
	if(count($result_txt) > 0){
		...
		header("location: /shop5test/chohyo_by_seihin_list.php");
		exit();
	}
...
// end 帳票登録　製品
	...
	if(count($result_txt) > 0){
		...
		header("location: /shop5test/chohyo_by_dia_list.php");
		exit();
	}
...
// end 帳票登録　dia
	...
	for ($product_id_array_i=0; $product_id_array_i < count($product_id_array); $product_id_array_i++) {
		...
		if($product_id_array[$product_id_array_i] != ''){
			// $default = "AND (`status` = 127)";
			$default = '';
			$query = "SELECT * FROM shouhin_copy20190708nishitani WHERE product_id=".$product_id_array[$product_id_array_i]." {$default} limit 1";
			$sql = mysql_query($query);
			$data = mysql_fetch_array($sql);
			...
			if($product_id != ''){
				...
				$sql = "UPDATE shouhin_copy20190708nishitani SET {$up_query}  WHERE product_id = {$product_id} limit 1";
				// var_dump($sql);
				// exit;
				// $result[] = ;
				// echo "<br>";
...
//完了時CW
	...
	if(count($result) > 0){
		...
		if(strstr($url_csv_create,'/shop5test/')){
			if($up_status == 137){
				//O到着時のCSV生成
		...
		//////////////////////////end　CSVファイル作成(O到着)////////////////////////////////////////
			...
			}//end if(strstr($url_csv_create,'/shop5test/'))
