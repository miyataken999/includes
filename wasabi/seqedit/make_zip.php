...
foreach ($csv as $key => $value) {
	...
	}elseif(($value == "hokan_place") OR ($value == "control") OR ($value == "zaiko") OR ($value == "Categories") OR ($value == "raku_image_name") OR ($value == "up_date") OR ($value == "related_words") OR ($value == "chain_type_select") OR ($value == "line_category") OR ($value == "mall_control")){
		//なし
	}else{
...
$seqInput = htmlspecialchars($_POST["seqInput"]);//product_id
...
// print "<br>".count($product_id_array)."<br>";

for ($seq_i=0; $seq_i < count($product_id_array); $seq_i++) {
//■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
//★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★対象のセレクタ指定★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★★
//■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■
	$product_id = $product_id_array[$seq_i];
	...
	// $query = "SELECT {$sql_key} FROM shouhin WHERE product_id={$product_id} LIMIT 1";
	$saisun_q = "SELECT COUNT(product_id) FROM products_detail WHERE product_id={$product_id}";
	$saisun_sql = mysql_query($saisun_q);
	$saisun_res = mysql_fetch_array($saisun_sql);
	if($saisun_res['COUNT(product_id)'] == 1){
		$products_detail_flag = "on";
		...
		$query = "SELECT {$sql_key} FROM (shouhin INNER JOIN auctions ON shouhin.product_id = auctions.product_id) INNER JOIN products_detail ON shouhin.product_id = products_detail.product_id  WHERE shouhin.product_id={$product_id} LIMIT 1";
	}else{
		$products_detail_flag = "off";
		$sql_key = $sql_key.",shouhin.initial_included,shouhin.novelty,shouhin.ws_import_date,shouhin.item_name,auctions.raku_goods_name,auctions.Starting_price";
		$query = "SELECT {$sql_key} FROM (shouhin INNER JOIN auctions ON shouhin.product_id = auctions.product_id) WHERE shouhin.product_id={$product_id} LIMIT 1";
	}
...
// mysql_error();
			...
			if(in_array($res['category_id'],$categori_array)){
				...
				// print $brand_query;
				$brand_sql = mysql_query('SET NAMES utf8');
				$brand_sql = mysql_query($brand_query);
			...
			if($res['Ring_size'] != ""){
				...
				$ws_output_sql = mysql_query('SET NAMES utf8');
				$ws_output_sql = mysql_query($ws_output_query);
...
$now = date("Y-m-d");
	for ($i=0; $i < count($seq_array); $i++) {
		if($seq_array[$i] != ""){
			// $qq = "UPDATE shouhin SET ws_import_date='{$now}' WHERE product_id={$seq_array[$i]} LIMIT 1";
			// print $qq;
			// $ss = mysql_query('SET NAMES utf8');
			// $ss = mysql_query($qq);
		}
	}
