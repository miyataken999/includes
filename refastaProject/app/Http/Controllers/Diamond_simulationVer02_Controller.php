...
class Diamond_simulationVer02_Controller extends Controller
{
    public function index(Request $request)
    {
        return view('diamond_simulationVer02.tools');
    }
    public function make_tool(Request $request)
    {
       // 金全データ取得
        $gold_price = tb_gold::all()->keyBy('gold_item');
        $gold_k24 = $gold_price->has('k24') ? $gold_price['k24']->gold_price : 0;
        $gold_k23 = $gold_price->has('k23') ? $gold_price['k23']->gold_price : 0;
        $gold_k22 = $gold_price->has('k22') ? $gold_price['k22']->gold_price : 0;
        $gold_k21 = $gold_price->has('k21') ? $gold_price['k21']->gold_price : 0;
        $gold_k20 = $gold_price->has('k20') ? $gold_price['k20']->gold_price : 0;
        $gold_k18 = $gold_price->has('k18') ? $gold_price['k18']->gold_price : 0;
        $gold_k14 = $gold_price->has('k14') ? $gold_price['k14']->gold_price : 0;
        $gold_k10 = $gold_price->has('k10') ? $gold_price['k10']->gold_price : 0;
        $gold_k9  = $gold_price->has('k9')  ? $gold_price['k9']->gold_price  : 0;
        $gold_k17 = $gold_price->has('k17') ? $gold_price['k17']->gold_price : 0;
        $gold_k12 = $gold_price->has('k12') ? $gold_price['k12']->gold_price : 0;
        $gold_k8  = $gold_price->has('k8')  ? $gold_price['k8']->gold_price  : 0;
        $gold_k5  = $gold_price->has('k5')  ? $gold_price['k5']->gold_price  : 0;
        // プラチナ全データ取得
        $platinum_price = tb_platinum::all()->keyBy('platinum_item');
        $platinum_pt1000 = $platinum_price->has('pt1000') ? $platinum_price['pt1000']->platinum_price : 0;
        $platinum_pt950 = $platinum_price->has('pt950') ? $platinum_price['pt950']->platinum_price : 0;
        $platinum_pt900 = $platinum_price->has('pt900') ? $platinum_price['pt900']->platinum_price : 0;
        $platinum_pt850 = $platinum_price->has('pt850') ? $platinum_price['pt850']->platinum_price : 0;
        $platinum_pt_pm = $platinum_price->has('pt_pm') ? $platinum_price['pt_pm']->platinum_price : 0;

        // パラジウム全データ取得
        $palladium_price = tb_palladium::all()->keyBy('palladium_item');
        $palladium_pd1000 = $palladium_price->has('pd1000') ? $palladium_price['pd1000']->palladium_price : 0;
        $palladium_pd950 = $palladium_price->has('pd950') ? $palladium_price['pd950']->palladium_price : 0;
        $palladium_pd900 = $palladium_price->has('pd900') ? $palladium_price['pd900']->palladium_price : 0;
        $palladium_pd500 = $palladium_price->has('pd500') ? $palladium_price['pd500']->palladium_price : 0;

         // シルバー全データ取得 
         $silver_price = tb_silver::all()->keyBy('silver_item');
         $silver_sv1000 = $silver_price->has('sv1000') ? $silver_price['sv1000']->silver_price : 0;
         $silver_sv925 = $silver_price->has('sv925') ? $silver_price['sv925']->silver_price : 0;
         $silver_sv900 = $silver_price->has('sv900') ? $silver_price['sv900']->silver_price : 0;

        // 歯科材のデータ取得、計算
        $calc_price = DB::select("SELECT `gold`, `palladium`, `silver` FROM `kinkai`.`tb_price` ORDER BY `id` DESC LIMIT 1");
        $dental_palladium_rate = $palladium_price->has('au12pd20') ? $palladium_price['au12pd20']->item_rate : 0;
        ...
        $calc_4 = floor(round(($calc_1+$calc_2+$calc_3)-200) * $dental_palladium_rate);
        $calc_4 = floor($calc_4 * 10000) / 10000;
          
        //  コンビ計算用レート取得
        $combi_price = tb_combi::all()->keyBy('combi_item');
        $c_rate_pt900_p = $combi_price->has('p900_k18_p') ? $combi_price['p900_k18_p']->item_rate : 0;
        $c_rate_pt900_h = $combi_price->has('p900_k18_h') ? $combi_price['p900_k18_h']->item_rate : 0;
        $c_rate_pt900_k = $combi_price->has('p900_k18_k') ? $combi_price['p900_k18_k']->item_rate : 0;
        $c_rate_pt850_p = $combi_price->has('p850_k18_p') ? $combi_price['p850_k18_p']->item_rate : 0;
        $c_rate_pt850_h = $combi_price->has('p850_k18_h') ? $combi_price['p850_k18_h']->item_rate : 0;
        $c_rate_pt850_k = $combi_price->has('p850_k18_k') ? $combi_price['p850_k18_k']->item_rate : 0;

        // コンビの計算
        $pt900_p =  floor(floor($gold_k18) * 0.1) + floor(floor($platinum_pt900) * 0.9);
        $pt900_p =  floor($pt900_p * $c_rate_pt900_p);
        $pt900_h =  floor(floor($gold_k18) / 2) + floor(floor($platinum_pt900) / 2);
        $pt900_h =  floor($pt900_h * $c_rate_pt900_h);
        $pt900_k =  floor(floor($gold_k18) * 0.9) + floor(floor($platinum_pt900) * 0.1);
        $pt900_k =  floor($pt900_k * $c_rate_pt900_k);

        $pt850_p =  floor(floor($gold_k18) * 0.1) + floor(floor($platinum_pt850) * 0.9);
        $pt850_p =  floor($pt850_p * $c_rate_pt850_p);
        $pt850_h =  floor(floor($gold_k18) / 2) + floor(floor($platinum_pt850) / 2);
        $pt850_h =  floor($pt850_h * $c_rate_pt850_h);
        $pt850_k =  floor(floor($gold_k18) * 0.9) + floor(floor($platinum_pt850) * 0.1);
        $pt850_k =  floor($pt850_k * $c_rate_pt850_k);
        ...
        return view('diamond_simulationVer02.index', $array);
    }
