...
<?
	...
	$query = " SELECT s.product_id , s.status , s.yahoo_sinaban , s.yahoo_title , s.raku_title , s.yahoo_sozai , s.item_name , s.line , s.yahoo_condition1 , s.remark
	FROM shouhin as `s`
	WHERE s.box_id = {$box_id} ORDER BY s.box_sort";
...
?>
	<tr id="toprow_<?=$result['product_id']?>" class="kaigai_<?=$kaigaiflag?> title_row">
		<td>
		...
		<!-- <td>Yタイトル</td> -->
		<td colspan="2"><?=$result['yahoo_title']?></td>
		<td>楽天</td>
		<!-- <td><a href="<?=$first_img_url?>" target="_blank"><img src="<?=$first_img_url?>" alt="" width="35px" height="35px" ></a></td> -->
		<td><a href="http://thumbnail.image.rakuten.co.jp/@0_mall/rfstore/cabinet/<?=$img_pass?>/<?=$pid?>_2.jpg" target="_blank"><img src="http://thumbnail.image.rakuten.co.jp/@0_mall/rfstore/cabinet/<?=$img_pass?>/<?=$pid?>_1.jpg" alt="" width="35px" height="35px" ></a></td>
	...
	<tr id="underrow_<?=$result['product_id']?>" class="kaigai_<?=$kaigaiflag?> detail_row">
		<td></td>
		<td>ST<br>
<?
...
$product_id = "";
?>
		<td><span class='dt_ttl'>コンディション</span><?=$result['yahoo_condition1']?></td>
		<td><span class='dt_ttl'>商品説明</span><?=$result['remark']?></td>
		<td>urlounge</td>
...
// var_dump('【ユーザID】'.$user_id);
?>
<form id="zip_edit_area" action="/include/raku_cabinet_shop5/status_up.php" method="get" accept-charset="utf-8">
