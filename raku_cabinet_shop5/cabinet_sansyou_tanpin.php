...
if($_GET[sinaban] != ""){
	$sinaban = $_GET[sinaban];
	$sinaban = strtoupper($sinaban);
}
?>
<h1>seq = <?=$seq?></h1>
<h1>品番 = <?=$sinaban?></h1>
<form action="./cabinet_sansyou_tanpin.php" method="get" accept-charset="utf-8">
	seq <input type="text" name="seq" value="<?=$seq?>" >
	<input type="submit" name="" value="実行">
</form>

<form action="./cabinet_sansyou_tanpin.php" method="get" accept-charset="utf-8">
	品番 <input type="text" name="sinaban" value="<?=$sinaban?>" >
	<input type="submit" name="" value="実行">
</form>
...
<?
if($seq != ""){
	$query = ' SELECT product_id , status , yahoo_sinaban , yahoo_title , box_id , raku_title , yahoo_sozai , item_name , line FROM shouhin WHERE product_id = '.$seq.' LIMIT 1';
}else{
	$query = ' SELECT product_id , status , yahoo_sinaban , yahoo_title , box_id , raku_title , yahoo_sozai , item_name , line FROM shouhin WHERE yahoo_sinaban = "'.$sinaban.'" LIMIT 1';
}
...
?>
<form id="zip_edit_area" action="/include/zip_shop5/makezip_tanpin.php" method="get" accept-charset="utf-8">
