...
// $syuppin_query = " UPDATE shouhin SET Exhibition_Date='{$syuppin_now}' , REG_EXHIBITOR='{$user_id}' , updated_at='{$now}' , updated_by='{$user_id}'  WHERE box_id={$box_id} ";
...
//★★★★楽天 item.csv★★★★
include $_SERVER["DOCUMENT_ROOT"]."/include/zip_shop5/process/rakuten/item_csv.php";
?>
...
<?
//★★★★楽天 item-cat.csv★★★★
include $_SERVER["DOCUMENT_ROOT"]."/include/zip_shop5/process/rakuten/item-cat_csv.php";
?>
...
<?
//★★★★楽天 select.csv★★★★
include $_SERVER["DOCUMENT_ROOT"]."/include/zip_shop5/process/rakuten/select_csv.php";
?>
...
<?
//★★★★楽天 next.csv★★★★
include $_SERVER["DOCUMENT_ROOT"]."/include/zip_shop5/process/rakuten/next_csv.php";
?>
...
//★★★★yahoo  csv★★★★
include $_SERVER["DOCUMENT_ROOT"]."/include/zip_shop5/process/yahoo/yahoo_auction_csv.php";
?>
...
$rakuten_zip_res = system("zip tmp/raku_zip/rakuten_{$box_name}.zip ./rakuten/next.csv ./rakuten/ritem/batch/*.csv {$image_pass}*.jpg ", $re_rakuten_zip); //{$box_name}/
$rakuten_zip = system("chmod -R 777 tmp/raku_zip/rakuten_{$box_name}.zip ", $re_rakuten_zip_mod);

if($box_name != ""){
	$yahoo_zip_res = system("zip tmp/yahoo_zip/yahoo_{$box_name}.zip -j ./tmp/yahoo/{$box_name}.csv {$image_pass}*.jpg ", $re_yahoo_zip); //{$box_name}/
...
<div id="link_box_back">
	<div id="link_box">
		<h3>ZIP作成に成功しました</h3>
		<a target="_blank" href='/include/zip_shop5/rakuten_download.php?box_name=<?=$box_name?>' >楽天ZIPダウンロード</a>
		<a target="_blank" href='/include/zip_shop5/yahoo_download.php?box_name=<?=$box_name?>' >ヤフオクZIPダウンロード</a>
	</div>
<?
//完了時のファイル
